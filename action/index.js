...
async function run() {
  try {
    ...
    const giphyUrl = `https://api.giphy.com/v1/gifs/random?api_key=${giphyApiKey}&tag=office&rating=g`;
    ...
    // Send the GIF as a separate attachment
    const gifPayload = {
      username: 'rahul-action-bot',
      icon_url: 'https://example.com/user-photo.png', // Replace with the actual user photo URL
      attachments: [
    ...
    await axios.post(slackWebhook, gifPayload);
